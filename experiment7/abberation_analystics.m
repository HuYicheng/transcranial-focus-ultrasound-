...
p_target = p_max_all(256,256,164);


[pmax_val, pmax_idx] = max(p_max_all(:));
[ix0, iy0, iz0] = ind2sub(size(p_max_all), pmax_idx);

p_filter1=zeros(size(p_max_all));
p_filter1(:,:,100:200)=p_max_all(:,:,100:200);

[pmax_val_filter1, pmax_idx_filter1] = max(p_filter1(:));
[ix0_filter1, iy0_filter1, iz0_filter1] = ind2sub(size(p_filter1), pmax_idx_filter1);

p_target_filter = p_max_all(ix0_filter1, iy0_filter1, iz0_filter1);

sliceViewer(p_max_all);


% ---------- 1) 绘制固定 z = zSlice 的 XY平面 ----------
zSlice = 164;                            % 例如取 z=280
