function phase_abberation()
    ...
    % 声学参数 (仅示例)
    c_bone = 2800;    % m/s
    rho_bone = 1900;  % kg/m^3
    c_brain = 1540;   % m/s
    rho_brain = 1050; % kg/m^3

    %% ==== 1. 读取并处理头骨 NIfTI ====
    fprintf('Reading NIfTI: %s\n', skull_nii_file);
    skull_data = niftiread(skull_nii_file);
    info = niftiinfo(skull_nii_file);
    disp('Size of skull_data:');
    disp(size(skull_data));  % should be [512, 512, 512]

    % (可检查 info.PixelDimensions 是否 [0.5 0.5 0.5], 单位 mm)
    disp('Pixel Dimensions (mm):');
    disp(info.PixelDimensions);

    % 简单阈值示例： skull_data > 500 => 骨 (此阈值 purely a guess!)
    % 实际应根据你的 CT 值范围确定更合理阈值
    bone_mask = (skull_data > 500);

    % 分配声速、密度
    sound_speed_map = single(c_brain) * ones(Nx, Ny, Nz, 'single');
    density_map     = single(rho_brain) * ones(Nx, Ny, Nz, 'single');

    sound_speed_map(bone_mask) = c_bone;
    density_map(bone_mask)     = rho_bone;
...
%     receiver_mask_3d(300, 350, 320) = 1;
%     
...
%     sensor.record = {'p_max_all','u_non_staggered','I_avg'};    
    ...
    for i = 1:32
        source.p{i} = source_mag*base_sig;
    end
